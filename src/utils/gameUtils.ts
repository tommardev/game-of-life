...
export const getTooltipText = (cell: CellState) => {
  if (cell.alive) {
    if (cell.neighbors < 2)
      return {
        title: 'Dying Next Turn',
        reason: 'Crippling loneliness. (<2 neighbors)',
      };
    if (cell.neighbors > 3)
      return {
        title: 'Dying Next Turn',
        reason: 'Suffocating overpopulation. (>3 neighbors)',
      };
    return {
      title: `Surviving (Age: ${cell.age})`,
      reason: 'Just right. The sweet spot of mediocrity.',
    };
  } else {
    if (cell.neighbors === 3)
      return {
        title: 'Being Born Next Turn',
        reason: 'Miraculous digital conception. (3 neighbors)',
      };
